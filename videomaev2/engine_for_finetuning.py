...
def merge_predictions(eval_path, num_tasks, method='prob'):
    assert method in ['prob', 'score']
    dict_feats = {}
    dict_label = {}
    dict_pos = {}
...
#            print('line:', line)
    ...
    final_top1, final_top5, final_top1_cls, final_top5_cls = compute_accuracy(preds)
    return final_top1 * 100, final_top5 * 100, final_top1_cls * 100, final_top5_cls * 100
...
def compute_accuracy(preds):
    ...
    # print('preds_0:', preds[0])
    # print('preds_0:', preds[0][0])
    # print('preds_0:', preds[0][1])

    # print('preds_0:', len(preds[0]))
    #print('preds_0:',len(preds[0]),preds[0][0][0].shape)
    final_top1 = 0
    final_top5 = 0
    # final_top1_cls = 0
    # final_top5_cls = 0

    final_top1_cls = [0 for i in range(len(preds[0][0][0]))]
    final_top5_cls = [0 for i in range(len(preds[0][0][0]))]

    #print(preds[0][0][0])
    cls_count = [0 for i in range(len(preds[0][0][0]))]
    #print(cls_count)
    #print(len(cls_count))    
    for pred, label in preds:
        pred = np.mean(pred, axis=0)
        top1 = (np.argmax(pred) == label) * 1.0
        top5 = (label in np.argsort(-pred)[:5]) * 1.0
        final_top1 += top1
        final_top5 += top5
        if label == np.argmax(pred):
            final_top1_cls[label] += 1
        if label in np.argsort(-pred)[:5]:
            final_top5_cls[label] += 1
        cls_count[label] += 1
    
    final_top1_cls = [top1_cls / count if count != 0 else 0 for top1_cls, count in zip(final_top1_cls, cls_count)]
    final_top5_cls = [top5_cls / count if count != 0 else 0 for top5_cls, count in zip(final_top5_cls, cls_count)]

    return final_top1/len(preds), final_top5/len(preds), np.mean(final_top1_cls), np.mean(final_top5_cls)