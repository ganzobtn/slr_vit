...
class VideoClsDataset(Dataset):
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            ...
            # #_, height, width, _ = buffer.shape
            # buffer[:,168:,160:216,:] = buffer[:,168:,112:168,:]
            # buffer[:,168:,80:136,:] = buffer[:,168:,56:112,:]
            # buffer[:,168:,56:80,:] = 0
            # buffer[:,168:,136:160,:] = 0
            # np.save(fname+'.npy',buffer)
            # #masking = np.zeros((height,width),dtype = bool)
            # #masking = np.ones((height,width),dtype = bool)
            #masking[height//2:, :width//2] = False
            #masking[:, width//2:] = True
            
            #buffer= np.where(masking[:,:,np.newaxis], buffer, 0)
